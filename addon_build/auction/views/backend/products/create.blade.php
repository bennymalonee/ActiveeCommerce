...
<form action="{{ route('auction_product_store.admin') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-md-8">
                    <select class="form-control aiz-selectpicker" name="category_ids[]" data-live-search="true" multiple required data-placeholder="{{ translate('Select Categories') }}">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ in_array($category->id, old('category_ids', [])) ? 'selected' : '' }}>{{ $category->getTranslation('name') }}</option>
                            @foreach ($category->childrenCategories as $child)
                                <option value="{{ $child->id }}" {{ in_array($child->id, old('category_ids', [])) ? 'selected' : '' }}>— {{ $child->getTranslation('name') }}</option>
                            @endforeach
                        @endforeach
                    </select>
                </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-md-8">
                    <input type="text" class="form-control aiz-date-range" name="auction_date_range" placeholder="{{ translate('Select Date Range') }}" data-time-picker="true" data-format="YYYY-MM-DD HH:mm" data-separator=" to " autocomplete="off" required>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="form-group row" id="flat_rate_input">
                <label class="col-md-3 col-from-label">{{ translate('Shipping Cost') }}</label>
                <div class="col-md-8">
                    <input type="number" class="form-control" name="flat_shipping_cost" value="{{ old('flat_shipping_cost', 0) }}" min="0" step="0.01">
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-8">
                    <label class="aiz-switch aiz-switch-success mb-0">
                        <input type="radio" name="button" value="publish" checked> {{ translate('Publish') }}
                    </label>
                    <label class="aiz-switch aiz-switch-success mb-0 ml-3">
                        <input type="radio" name="button" value="unpublish"> {{ translate('Unpublish') }}
                    </label>
                </div>
            </div>
